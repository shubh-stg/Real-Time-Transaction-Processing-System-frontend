...
export default function SendMoney() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!recieverId.trim()) {
      setStatus("⚠️ Please Enter a Valid Receiver ID ");
      return;
    }
        ...
        setTimeout(async () => {
          try {
            ...
            if (notifications.length > 0) {
              ...
              if (
                latestNotification?.message?.includes("You successfully sent ₹")
              ) {
                setStatus(
                  "✅ Transaction completed successfully! Redirecting..."
                );
              } else if (
                latestNotification?.message?.includes("Transaction failed:")
              ) {
                setStatus("❌ Transaction failed. Redirecting...");
              } else {
                setStatus(
                  "⏳ Transaction processing. Check notifications. Redirecting..."
                );
              }
            } else {
            ...
            setStatus(
              "⏳ Transaction submitted. Check notifications. Redirecting..."
            );
            setTimeout(() => navigate("/user-dashboard"), 2000);
          }
        }, 6000);
